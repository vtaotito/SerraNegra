...
export function BITopbar() {
  ...
  return (
    <header className="flex items-center justify-between gap-4 flex-wrap pb-4 border-b border-cockpit-border">
      <div className="flex items-center gap-3">
        <div className="p-2 rounded-xl bg-cockpit-accent/10">
      ...
      <div className="flex items-center gap-3">
        <div className="relative z-[60]" ref={pickerRef}>
          ...
          >
            ...
            <span className="text-gray-600 hidden sm:inline">{rangeLabel}</span>
            <span className="text-gray-600 sm:hidden">
              {preset === "custom" ? "Personalizado" : PRESETS[preset].label}
            </span>
            <ChevronDown className={`w-3.5 h-3.5 text-cockpit-muted transition-transform duration-200 ${pickerOpen ? "rotate-180" : ""}`} />
          </button>

          {pickerOpen && (
            <div className="absolute top-full right-0 mt-2 w-[calc(100vw-2rem)] sm:w-[440px] rounded-xl border border-cockpit-border bg-white shadow-2xl shadow-black/10 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
              <div className="flex items-center justify-between px-4 py-3 border-b border-cockpit-border bg-gray-50/50">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-cockpit-accent" />
                  <span className="text-sm font-semibold text-gray-900">Período de análise</span>
                </div>
                <button type="button" onClick={() => setPickerOpen(false)} className="p-1 rounded-md text-cockpit-muted hover:text-gray-900 hover:bg-black/5 transition-colors" aria-label="Fechar">
                  <X className="w-4 h-4" />
                </button>
              </div>
                    ...
                    return (
                      <button key={key} type="button" onClick={() => handlePreset(key)}
                        className={`group flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-all ${
                          isActive ? "bg-cockpit-accent/15 border border-cockpit-accent/30 text-cockpit-accent" : "border border-transparent text-gray-600 hover:bg-black/5 hover:border-cockpit-border"
                        }`}>
                        <div>
