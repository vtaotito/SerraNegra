              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={filtered.slice(0, 12)} layout="vertical" barCategoryGap="15%">
                    ...
                    <YAxis dataKey="group" type="category" tick={{ ...chartAxisTick("md"), fontWeight: 600 }} width={40} />
                      ...
                      content={(props: {
                        ...
                        return (
                          <BiChartTooltip
                            ...
                            formatValue={(name, v) => (name === "Linhas" ? fmtNum(v) : fmtBRL(v))}
                          />
                        );
                      }}
                    />
                    <Bar dataKey="count" name="Linhas" radius={[4, 4, 0, 0]} fill={CHART_SERIES_PRIMARY} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={filtered.filter((g) => g.descontoMedio > 0).slice(0, 10)}>
                    ...
                    <YAxis yAxisId="left" tick={chartAxisTick("md")} tickFormatter={(v: number) => `${v.toFixed(0)}%`} />
                    ...
                    <Tooltip
                      content={
                        <BiChartTooltip
                          variant="cockpit"
                          formatValue={(name, v) =>
                            name?.includes("Desc") || name?.includes("%") ? `${v.toFixed(1)}%` : fmtBRL(v)
                          }
                        />
                      }
                    />
          ...
          <>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_AXIS_LINE} />
                  <XAxis dataKey="x" type="number" name="Qtd" tick={chartAxisTick("md")}
                    label={{ value: "Qtd Vendida", position: "bottom", ...CHART_AXIS_LABEL_PROPS }} />
                  <YAxis dataKey="y" type="number" name="Preço Médio" tick={chartAxisTick("md")}
                    tickFormatter={(v: number) => fmtBRL(v, 0)} />
                  <ZAxis dataKey="z" range={[100, 800]} name="Faturamento" />
                  <Tooltip
                    content={(props: {
                      active?: boolean;
                      payload?: readonly { payload?: { x: number; y: number; z: number; name: string; desc: number } }[];
                    }) => {
                      ...
                      return (
                        <BiChartTooltip
                          ...
                          formatValue={(name, v) => {
                            ...
                            if (name === "Desc. Médio %") return `${v.toFixed(1)}%`;
