...
import {
  ...
  XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine, ScatterChart, Scatter, ZAxis,
} from "recharts";
...
import {
  CHART_SERIES_PRIMARY, CHART_AXIS_LINE, CHART_MUTED,
  chartAxisTick, formatYAxisCompact,
...
type AnalisePanel = "resumo" | "lista" | "indicadores";

function resolvePanel(raw: string | null): AnalisePanel {
  if (raw === "lista" || raw === "indicadores" || raw === "resumo") return raw;
  return "resumo";
}

const WEEKDAY_NAMES = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"];
/** Grid alinhado ao cabeçalho da tabela de pedidos (virtualizada). */
const ORDER_ROW_GRID =
  "min-w-[800px] grid grid-cols-[28px_72px_88px_minmax(140px,1.2fr)_minmax(88px,0.9fr)_52px_72px_88px_76px_minmax(96px,1fr)] gap-x-2 items-center px-2 sm:px-3";
...
export function PedidosAnaliseView({ embedded = false }: { embedded?: boolean }) {
  return (
    <Suspense fallback={<LoadingSkeleton rows={8} />}>
      <PedidosAnaliseContent embedded={embedded} />
    </Suspense>
  );
}

function PedidosAnaliseContent({ embedded }: { embedded: boolean }) {
  ...
  const setPanel = useCallback(
    (next: AnalisePanel) => {
      ...
      if (next === "resumo") params.delete("panel");
      else params.set("panel", next);
      if (!params.get("view")) params.set("view", "analise");
  ...
  const expandAll = useCallback(() => {
    ...
    for (const v of rowVirtualizer.getVirtualItems()) {
      const o = filtered[v.index];
      if (o) next.add(o.doc_entry);
    }
    setExpanded(next);
  }, [rowVirtualizer, filtered]);
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        {!embedded ? (
          ...
          <p className="text-xs sm:text-sm text-cockpit-muted flex items-center gap-1.5 flex-wrap min-w-0">
            ...
            <span className="truncate">{rangeLabel}</span>
            <span className="text-cockpit-border hidden sm:inline">·</span>
            <strong className="text-gray-700">{data?.total ?? 0}</strong> registros SAP
          </p>
        )}
        <div className="flex items-center gap-2">
      ...
      {cardCodeFromUrl && clienteFilter && (
        <div className="rounded-xl border border-cockpit-accent/30 bg-cockpit-accent/5 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          ...
          <Link href="/pedidos?view=analise" className="text-sm font-medium text-cockpit-accent hover:text-cockpit-accent/80 motion-safe:transition-colors flex items-center gap-1.5">
        ...
        >
          {([
            ...
            <button
              ...
              className={cn(
                "flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium motion-safe:transition-colors min-h-[44px] sm:min-h-0",
                panel === key
      ...
      {panel === "resumo" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <ChartCard title="Evolução diária" icon={TrendingUp}>
            {chartByDay.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartByDay}>
                  ...
                  <XAxis
                    ...
                    minTickGap={28}
                  />
                  <YAxis tick={chartAxisTick("sm")} tickFormatter={(v: number) => formatYAxisCompact(v)} />
                  ...
                  {dailyMedian > 0 && (
                    <ReferenceLine
                      ...
                      stroke={CHART_MUTED}
                      strokeDasharray="4 4"
                      label={{ value: "Mediana", fill: CHART_MUTED, fontSize: 10, position: "insideTopRight" }}
                    />
                  )}
                  <Bar dataKey="valor" name="Valor" fill={CHART_SERIES_PRIMARY} radius={[3, 3, 0, 0]} />
                </ComposedChart>
              </ResponsiveContainer>
            )}
          </ChartCard>

          <ChartCard title="Acumulado no período" icon={DollarSign}>
            {cumulativeData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={cumulativeData}>
                  ...
                  <YAxis tick={chartAxisTick("sm")} tickFormatter={(v: number) => formatYAxisCompact(v)} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
                  <Bar dataKey="diario" name="Diário" fill={`${CHART_SERIES_PRIMARY}55`} radius={[2, 2, 0, 0]} />
                  <Line dataKey="acumulado" name="Acumulado" stroke="#059669" strokeWidth={2} dot={false} />
                </ComposedChart>
              </ResponsiveContainer>
            )}
          </ChartCard>

          <ChartCard title="Status dos pedidos" icon={PieChartIcon}>
            {statusData.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
          ...
          <ChartCard title="Top clientes" icon={Users}>
            {topClients.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topClients} layout="vertical" barCategoryGap="15%">
                  ...
                  <XAxis type="number" tick={chartAxisTick("sm")} tickFormatter={(v: number) => formatYAxisCompact(v)} />
                  <YAxis dataKey="nome" type="category" tick={chartAxisTick("sm")} width={110} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
                  <Bar dataKey="valor" name="Valor" radius={[0, 4, 4, 0]} fill={CHART_SERIES_PRIMARY} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </ChartCard>

          <ChartCard title="Distribuição de ticket" icon={BarChart3}>
            {histData.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={histData}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_AXIS_LINE} />
                  <XAxis dataKey="faixa" tick={chartAxisTick("sm")} />
                  <YAxis tick={chartAxisTick("sm")} allowDecimals={false} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
          ...
          <ChartCard title="Box plot do ticket" icon={Minus} height="h-auto min-h-[14rem]">
            {!stats ? (
              <EmptyChart />
            ) : (
              <div className="flex flex-col justify-center h-full py-4">
                <BoxPlotVisual
                  ...
                  mean={stats.mean}
                />
                <div className="mt-4 grid grid-cols-3 gap-2 text-center text-[10px] text-cockpit-muted">
                  <div>
                    <p className="uppercase tracking-wider">IQR</p>
                    <p className="text-sm font-semibold text-gray-800 tabular-nums">{fmtBRL(stats.iqr)}</p>
                  </div>
                  <div>
                    <p className="uppercase tracking-wider">Desvio</p>
                    <p className="text-sm font-semibold text-gray-800 tabular-nums">{fmtBRL(stats.stdDev)}</p>
                  </div>
                  <div>
                    <p className="uppercase tracking-wider">CV</p>
                    <p className="text-sm font-semibold text-gray-800 tabular-nums">{(stats.cv * 100).toFixed(0)}%</p>
                  </div>
                </div>
              </div>
            )}
      ...
      {panel === "indicadores" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <ChartCard title="Por dia da semana" icon={CalendarDays}>
            {weekdayData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weekdayData}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_AXIS_LINE} />
                  <XAxis dataKey="dia" tick={chartAxisTick("sm")} />
                  <YAxis tick={chartAxisTick("sm")} tickFormatter={(v: number) => formatYAxisCompact(v)} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
                  <Bar dataKey="valor" name="Valor" radius={[3, 3, 0, 0]}>
                    {weekdayData.map((d) => (
                      <Cell key={d.dia} fill={WEEKDAY_COLORS[d.idx] ?? CHART_SERIES_PRIMARY} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <ChartCard title="Por vendedor" icon={Briefcase}>
            {salesPersonData.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={salesPersonData} layout="vertical" barCategoryGap="12%">
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_AXIS_LINE} horizontal={false} />
                  <XAxis type="number" tick={chartAxisTick("sm")} tickFormatter={(v: number) => formatYAxisCompact(v)} />
                  <YAxis dataKey="vendedor" type="category" tick={chartAxisTick("sm")} width={100} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
                  {spMedianAll > 0 && (
                    <ReferenceLine x={spMedianAll} stroke={CHART_MUTED} strokeDasharray="4 4" />
                  )}
                  <Bar dataKey="valor" name="Valor" fill={CHART_SERIES_PRIMARY} radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </ChartCard>

          <ChartCard title="Lead time (pedido → entrega)" icon={Clock}>
            {leadTime.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={leadTime}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_AXIS_LINE} />
                  <XAxis dataKey="dias" tick={chartAxisTick("sm")} label={{ value: "dias", position: "insideBottomRight", offset: -4, fill: CHART_MUTED, fontSize: 10 }} />
                  <YAxis tick={chartAxisTick("sm")} allowDecimals={false} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
                  {leadTimeMedian > 0 && (
                    <ReferenceLine
                      ...
                      stroke="#f59e0b"
                      strokeDasharray="4 4"
                      label={{ value: `Med ${leadTimeMedian.toFixed(0)}d`, fill: "#f59e0b", fontSize: 10 }}
                    />
                  )}
                  <Bar dataKey="count" name="Pedidos" fill="#8b5cf6" radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </ChartCard>

          <ChartCard title="Itens × valor" icon={Package}>
            {scatter.length === 0 ? (
              <EmptyChart />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_AXIS_LINE} />
                  <XAxis type="number" dataKey="itens" name="Itens" tick={chartAxisTick("sm")} allowDecimals={false} />
                  <YAxis type="number" dataKey="valor" name="Valor" tick={chartAxisTick("sm")} tickFormatter={(v: number) => formatYAxisCompact(v)} />
                  <ZAxis range={[40, 40]} />
                  <Tooltip content={<BiChartTooltip variant="cockpit" />} />
                  <Scatter name="Pedidos" data={scatter} fill={CHART_SERIES_PRIMARY} fillOpacity={0.55} />
                </ScatterChart>
              </ResponsiveContainer>
            )}
          </ChartCard>
      ...
      {/* ═══ Tabela de pedidos (virtualizada) ═══ */}
      <div className="rounded-xl border border-cockpit-border bg-white overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 border-b border-cockpit-border bg-gray-50/80 gap-2">
          <p className="text-[11px] sm:text-xs text-cockpit-muted">
            <strong className="text-gray-800">{filtered.length}</strong> pedido{filtered.length !== 1 ? "s" : ""}{" "}
            <span className="text-cockpit-muted/80">(lista virtualizada)</span>
          </p>
          {filtered.length > 0 && (
        ...
        <div ref={tableScrollRef} className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-320px)] text-sm">
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-cockpit-muted px-4">
            ...
            <>
              <div
                className={`${ORDER_ROW_GRID} py-2.5 border-b border-cockpit-border bg-gray-50/95 text-[10px] uppercase tracking-wider text-cockpit-muted font-semibold sticky top-0 z-20 backdrop-blur-sm shadow-[0_1px_0_var(--cockpit-border)]`}
                role="row"
              >
                <div className="w-7 shrink-0" aria-hidden />
                <div role="columnheader" className="text-left cursor-pointer select-none hover:text-gray-700 min-w-0" onClick={() => toggleSort("doc_num")}>
                  <span className="inline-flex items-center gap-1">Nº <SortIcon field="doc_num" /></span>
                </div>
                <div role="columnheader" className="text-left cursor-pointer select-none hover:text-gray-700 min-w-0" onClick={() => toggleSort("doc_date")}>
                  <span className="inline-flex items-center gap-1">Data <SortIcon field="doc_date" /></span>
                </div>
                <div role="columnheader" className="text-left cursor-pointer select-none hover:text-gray-700 min-w-0" onClick={() => toggleSort("card_name")}>
                  <span className="inline-flex items-center gap-1">Cliente <SortIcon field="card_name" /></span>
                </div>
                <div role="columnheader" className="text-left hidden lg:block min-w-0">Local</div>
                <div role="columnheader" className="text-center cursor-pointer select-none hover:text-gray-700" onClick={() => toggleSort("num_lines")}>
                  <span className="inline-flex items-center gap-1 justify-center">Itens <SortIcon field="num_lines" /></span>
                </div>
                <div role="columnheader" className="text-right cursor-pointer select-none hover:text-gray-700" onClick={() => toggleSort("total_quantity")}>
                  <span className="inline-flex items-center gap-1 justify-end w-full">Qtd <SortIcon field="total_quantity" /></span>
                </div>
                <div role="columnheader" className="text-right cursor-pointer select-none hover:text-gray-700" onClick={() => toggleSort("doc_total")}>
                  <span className="inline-flex items-center gap-1 justify-end w-full">Valor <SortIcon field="doc_total" /></span>
                </div>
                <div role="columnheader" className="text-center">Status</div>
                <div role="columnheader" className="text-left hidden xl:block min-w-0">Vendedor</div>
              </div>

              <div className="relative w-full min-w-[800px]" style={{ height: `${rowVirtualizer.getTotalSize()}px` }}>
                {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                  ...
                  return (
                    <div
                      ...
                      className="absolute left-0 top-0 w-full border-b border-cockpit-border/30 bg-white"
                    ...
                    >
                      <div
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" || e.key === " ") {
                            e.preventDefault();
                            toggleExpand(order.doc_entry);
                          }
                        }}
                        className={`${ORDER_ROW_GRID} py-2.5 cursor-pointer motion-safe:transition-colors ${
                      ...
                      >
                        ...
                        <div className="text-gray-800 min-w-0 truncate font-medium" title={`${order.card_name} (${order.card_code})`}>
                          ...
                          <span className="block text-[10px] text-gray-400 font-mono truncate">{order.card_code}</span>
                        </div>
                        <div className="text-xs text-gray-500 min-w-0 truncate hidden lg:block" title={loc}>
                          {loc && loc !== "—" ? (
                        ...
                        <div className="text-right tabular-nums font-medium text-gray-800 text-xs sm:text-sm">{fmtQty(qty)}</div>
                        <div className="text-right tabular-nums font-bold text-gray-900 text-xs sm:text-sm">{fmtBRL(Number(order.doc_total) || 0)}</div>
                        <div className="text-center">
                        ...
                        <div className="text-xs text-gray-600 min-w-0 truncate hidden xl:block" title={vendorName}>{vendorName}</div>
                      </div>
                      {isExpanded && (
                        isLoadingLines ? (
        ...
        {filtered.length > 0 && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between px-3 sm:px-4 py-2 border-t border-cockpit-border bg-cockpit-accent/[0.03] text-xs gap-1">
            <span className="text-cockpit-muted">{filtered.length} pedidos (filtro)</span>
            <div className="flex items-center gap-3 sm:gap-6 tabular-nums flex-wrap">
              <span className="text-gray-600 hidden sm:inline">
                Itens: <strong className="text-gray-800">{filtered.reduce((s, o) => s + (o.lines?.length ?? o.num_lines ?? 0), 0)}</strong>
              </span>
              <span className="text-gray-600">
                Qtd: <strong className="text-gray-800">{fmtQty(filtered.reduce((s, o) => s + (Number(o.total_quantity) || 0), 0))}</strong>
              </span>
              <span className="text-cockpit-accent font-bold">{fmtBRL(filtered.reduce((s, o) => s + (Number(o.doc_total) || 0), 0))}</span>
            </div>
          </div>
        )}
      </div>
...
function ChartCard({
  ...
  return (
    <div className="rounded-xl border border-cockpit-border bg-white p-4 sm:p-5 shadow-sm">
