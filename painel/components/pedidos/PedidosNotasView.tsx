...
function PedidoLink({ docNum }: { docNum: number | null }) {
  ...
  return (
    <Link
      href={`/pedidos?view=analise&panel=lista&search=${encodeURIComponent(String(docNum))}`}
      ...
      title="Ver pedido vinculado na Análise"
    >
...
export function PedidosNotasView({ embedded = false }: { embedded?: boolean }) {
  return (
    <Suspense fallback={<LoadingSkeleton rows={8} />}>
      <PedidosNotasContent embedded={embedded} />
    </Suspense>
  );
}

function PedidosNotasContent({ embedded }: { embedded: boolean }) {
  ...
  const [vinculoFilter, setVinculoFilter] = useState<"ALL" | "linked" | "unlinked">(
    searchFromUrl ? "linked" : "ALL",
  );

  useEffect(() => {
    if (searchFromUrl) {
      setSearch(searchFromUrl);
      setVinculoFilter("linked");
    }
  }, [searchFromUrl]);
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        {embedded && (
          <p className="text-sm text-cockpit-muted">Carregando notas fiscais do SAP B1...</p>
        )}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        {!embedded ? (
          ...
          <p className="text-xs sm:text-sm text-cockpit-muted flex items-center gap-1.5 flex-wrap min-w-0">
            ...
            <span>
              <strong className="text-gray-700">{fmtNum(allDocs.length)}</strong> NFs ·{" "}
              <strong className="text-gray-700">{fmtNum(linkedCount)}</strong> com pedido
              {cancelledCount > 0 && <> · {cancelledCount} canceladas</>}
            </span>
          </p>
        )}
      ...
      {searchFromUrl && (
        <div className="rounded-xl border border-cockpit-accent/30 bg-cockpit-accent/5 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <p className="text-sm text-gray-700">
            Notas do pedido: <strong className="text-cockpit-accent">#{searchFromUrl}</strong>
          </p>
          <Link
            href="/pedidos?view=notas"
            className="text-sm font-medium text-cockpit-accent hover:text-cockpit-accent/80 motion-safe:transition-colors inline-flex items-center gap-1.5"
          >
      ...
      <footer className="text-center text-[10px] text-cockpit-muted py-2">
        Origem: SAP B1 · Sincronização horária · vínculo NF ↔ Pedido via BaseDocNum
      </footer>
    </div>
  );
}
