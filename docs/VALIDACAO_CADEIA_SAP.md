...
```bash
# Login
curl -X POST https://sap-garrafariasnegra-sl.skyinone.net:50000/b1s/v1/Login \
...
**Listar pedidos**:
```bash
curl -X GET "https://sap-garrafariasnegra-sl.skyinone.net:50000/b1s/v1/Orders?\$top=5&\$select=DocEntry,DocNum,CardCode" \
...
```bash
# Health do nginx
curl -i http://31.97.174.120:8080/health

# API via nginx (rota /api)
curl -i "http://31.97.174.120:8080/api/orders?limit=5"

# Frontend via nginx (rota /)
curl -i http://31.97.174.120:8080/
```
...
2. **Trigger sync manual** (do seu PC ou servidor):
   ...
   ```
3. **Abrir painel**: `http://31.97.174.120:8080/`
