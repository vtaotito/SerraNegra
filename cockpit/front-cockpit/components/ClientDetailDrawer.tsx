...
export function ClientDetailDrawer({ client, open, onClose }: ClientDetailDrawerProps) {
  ...
  const pedidosUrl = `/pedidos?cardCode=${encodeURIComponent(client.codigo)}&clientName=${encodeURIComponent(client.cliente)}`;
  ...
  return (
    <>
      ...
      <aside
        ...
        role="dialog"
        className={clsx(
          "fixed top-0 right-0 z-50 w-full max-w-md h-full bg-cockpit-surface shadow-2xl border-l border-cockpit-border flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]",
          open ? "translate-x-0" : "translate-x-full"
        )}
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-cockpit-border bg-cockpit-bg/50 shrink-0">
