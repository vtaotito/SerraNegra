...
export default function MargensPage() {
  ...
  if (error) return <div className="space-y-6"><div><h1 className="text-2xl font-bold text-gray-900">CMV / Margens</h1></div><ErrorState message={error} onRetry={() => { r1(); r2(); }} /></div>;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">CMV / Margens</h1>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-cockpit-border">
              {filtered.length === 0 ? (
                ...
                <tr key={r.categoria} className="hover:bg-black/5">
                  <td className="px-4 py-3 font-medium text-gray-900">{r.categoria}</td>
                  <td className="px-4 py-3 text-right text-gray-600">{r.itens}</td>
