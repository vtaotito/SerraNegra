...
/**
 * Barra de navegacao inferior (mobile-first). Fixa na base da viewport, some
          ...
          return (
            <li key={item.href} className="flex-1">
              <Link
                ...
                className={cn(
                  "relative flex min-h-[3.5rem] flex-col items-center justify-center gap-0.5 px-1 py-1.5 text-[11px] font-medium transition-colors",
              ...
              >
                ...
                {item.label}
                {isActive && (
                  <span className="absolute inset-x-4 top-0 h-0.5 rounded-full bg-gsn-brand" />
                )}
              </Link>
            </li>
          );
