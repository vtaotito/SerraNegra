...
const NEXT_EVENTS: Record<string, { event: string; label: string; icon: any }[]> = {
  [OrderStatus.A_SEPARAR]: [
    { event: "INICIAR_SEPARACAO", label: "Iniciar Separação", icon: Play },
  ],
  [OrderStatus.EM_SEPARACAO]: [
    { event: "FINALIZAR_SEPARACAO", label: "Finalizar Separação", icon: CheckCircle2 },
  ],
  [OrderStatus.CONFERIDO]: [
    { event: "SOLICITAR_COTACAO", label: "Solicitar Cotação", icon: Play },
  ],
  [OrderStatus.AGUARDANDO_COTACAO]: [
    { event: "CONFIRMAR_COTACAO", label: "Confirmar Cotação", icon: CheckCircle2 },
  ],
          ...
          {isLoadingHistory ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Eventos aparecerão aqui conforme o pedido avança no fluxo.
              </p>
            </div>
          ) : (
